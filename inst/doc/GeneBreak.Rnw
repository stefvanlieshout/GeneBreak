%\VignetteIndexEntry{Introduction to GeneBreak}
...
This is a short tutorial on how to use the \pkg{GeneBreak} package. It covers an example run using included copy number data of one chromosome from 200 samples. The samples are part of the CAIRO study described in REF. Let's start by loading the package.
...
\begin{figure}[htbp]
\centering
\includegraphics [width = 360pt]{figure_SI.pdf}
\caption{\textit{GeneBreak} geneAnnotation }
\label{fig:geneAnnotation}
\end{figure}
